...
from src.tests.test_image_sizes import ImageSizesTest
...
class NodeTest:
    def __init__(
        ...
        if test_selection["tensor_types"]:
            tensor_test(max_branches=self.max_branches)

        image_size_test = ImageSizesTest(
            self.target_node["instance"],
            self.target_node["tensor_input_fields"],
            self.target_node["input_types"],
        )
        if test_selection["image_sizes"]:
            image_size_test(max_branches=self.max_branches)

    def parse_tensor_input_fields(self, INPUT_TYPES):
        """Parse the input types for the node

        Example of INPUT_TYPES:
